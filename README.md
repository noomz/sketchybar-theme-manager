...
| `stm adopt [slug]` | Make an existing bar stm-compatible (default slug: `mine`) |
| `stm backup [name]` | Snapshot stm-owned files (pass `--all` for the whole config dir) |
| `stm restore [name]` | Restore a snapshot (latest auto snapshot if the name is omitted) |
| `stm backups` | List snapshots |
| `stm help` | Show usage |
...
| `--no-backup` | Don't take an auto snapshot on apply (or adopt) |
...
| `--all` | (`backup`) Snapshot the whole config dir, not just stm-owned files |
...
`colors.lua` on apply**. The one exception is `stm adopt`, which may rename
`colors.lua` to `colors_user.lua` once and leave a tiny overlay wrapper behind.
The generated file is a plain Lua module:
...
### Adopt an existing bar

`import` only writes a palette. `adopt` finishes the job: freeze the tree,
import colours, scrape bar chrome / item slots into the same palette, wire a
Lua overlay if needed, write a minimal `stm.config.toml` when one is missing,
and apply.

```sh
stm adopt                # default slug: mine
stm current              # mine
stm apply nord           # bar colours change
stm restore pre-adopt    # whole tree back to the frozen original
```

Lua: if `colors.lua` does not already mention `colors_generated`, adopt
renames it to `colors_user.lua` and writes a tiny trusted wrapper as
`colors.lua`. `require("colors")` keeps working. stm will not rewrite that
wrapper again. `--no-wire` skips the rename.

Bash: no wrap. `colors.sh` is already the apply target. `sketchybarrc` stays
a shell script — adopt never rewrites rc to SbarLua.

Layout scrape is conservative and data-only. It never edits item files. If
nothing matches, the `[layout]` / `[items]` sections are omitted.

`--dry-run` prints every step and writes nothing. `--no-backup` skips the
snapshot; that is allowed and unwise.
...
| `{{bar_position}}` `{{bar_height}}` … | a single layout value |
| `{{layout_lua}}` / `{{items_lua}}` / `{{layout_bash}}` | generated layout blocks |
...
`apply` writes an automatic snapshot of stm-owned files (the generated colour
and layout overlays, plus `.stm-state`) into `.stm-backups/` before it changes
anything, and keeps the newest five. Named snapshots are never pruned.

```sh
stm backup --all working      # whole config dir: items, plugins, sketchybarrc
stm backup before-experiment  # stm-owned files only
stm backups                   # list
stm restore                   # newest auto snapshot
stm restore working           # that named snapshot (scope is recorded)
```

`--all` is how you freeze a bar that already works, before the first `apply`.
Restore follows the snapshot's own scope — there is no second `--all` flag.
`.git/` and `.stm-backups/` are never copied into a snapshot, and a symlink
that points outside the config dir is skipped.

### Layout and item positions

A palette may optionally declare bar chrome and which slot each item sits in.
`stm` writes a generated overlay and **never edits your item files**.
...
Values are data: enums (`top`/`bottom`, `on`/`off`, `left`/`right`/`center`)
and small integers. Unknown keys are an error. Bundled palettes stay
colour-only — item names are not universal.

Lua configs get `layout_generated.lua`:

```lua
local layout = {}
local ok, generated = pcall(require, "layout_generated")
if ok and type(generated) == "table" then
  layout = generated
end

sbar.bar({
  position = (layout.bar and layout.bar.position) or "top",
  height   = (layout.bar and layout.bar.height) or 32,
  color    = colors.bar_bg,
})
...
sbar.add("item", "clock", { position = pos("clock", "right") })
```
